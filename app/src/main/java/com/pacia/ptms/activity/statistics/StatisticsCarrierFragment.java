 ...
 * 安全统计 根据承运商
 */
public class StatisticsCarrierFragment extends BaseFragment implements BaseQuickAdapter.RequestLoadMoreListener {
    private static final String TAG = "StatisticsCarrierFragme";

    public static Fragment getInstance(String msg) {
        StatisticsCarrierFragment fragment = new StatisticsCarrierFragment();
    ...
    public void initData() {
        adapter = new StatisticsAdapter(fContext, "carrier", R.layout.item_rv_statis_list, list_sta);
        ...
        swipLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                isRefresh = true;
                page = 1;
                getStatisticsList(type, date, page, rows);
            }
        });
    ...
    private void getStatisticsList(String type, String date, int page, int rows) {
        ServiceFactory.getService(ApiService.class)
                .queryStatisticsByCarrier(type, date, page, rows)
    ...
    private void showStatisticsList(List<StatisticsBean> list, int total) {
        ...
        cdpStart = new CustomDatePicker(false, context, new CustomDatePicker.ResultHandler() {
            ...
            public void handle(String time) { // 回调接口，获得选中的时间
                if (!"".equals(time)) {
                    ...
                    tv.setText(date);
                    getStatisticsList(type, DateUtils.yearToTimeStamp(date) + "", page, rows);
                }
            }
