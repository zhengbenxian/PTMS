...
public class CustomDatePicker {
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler,
                            String startDate, String endDate, boolean isSingle) {
        if (isValidDate(startDate, "yyyy-MM-dd HH:mm") && isValidDate(endDate, "yyyy-MM-dd HH:mm")) {
            canAccess = true;
            this.context = context;
            this.handler = resultHandler;
    ...
    public CustomDatePicker(boolean isShowDay, Context context, ResultHandler resultHandler,
                            String startDate, String endDate) {
        if (isValidDate(startDate, "yyyy-MM-dd HH:mm") && isValidDate(endDate, "yyyy-MM-dd HH:mm")) {
            canAccess = true;
            this.context = context;
    ...
    private void initTimer() {
        initArrayList();
        Log.e(TAG, "initTimer: " + spanYear + " " + spanMon + " " + spanDay + " " +
                spanHour);
        if (spanYear) {
            ...
            Log.e(TAG, "initTimer: " + day.toString());
            if ((scrollUnits & SCROLL_TYPE.HOUR.value) != SCROLL_TYPE.HOUR.value) {
                hour.add(formatTimeUnit(startHour));
            } else {
                for (int i = startHour; i <= MAX_HOUR; i++) {
